...
if not os.path.isdir(preprocessed_folder+"Images\\"):
    os.makedirs(preprocessed_folder+"Images\\")
if not os.path.isdir(preprocessed_folder+"Segmentations\\"):
    os.makedirs(preprocessed_folder+"Segmentations\\")
...
def my_save(case, processed_file, i):
    s_case = str(case)
    type = ["Images\\", "Segmentations\\"]
    path_save = preprocessed_folder+type[i]+s_case
    file = (processed_file/255).astype(np.float16)
    np.save(path_save, file)
